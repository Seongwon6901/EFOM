...
MIN_FEED_TPH_ACTIVE = 0.1
MIN_ACTIVE_RCOT_NAPH = 800.0
MIN_ACTIVE_RCOT_GAS  = None

# Targets (use *_uncracked_* if your recycle logic expects them)
TARGET_COLS: List[str] = [
    'Ethylene_prod_t+1','Propylene_prod_t+1','MixedC4_prod_t+1','RPG_prod_t+1',
    'Ethane_uncracked_t+1','Propane_uncracked_t+1'
]
...
SHORT_MAP: Dict[str, str] = {
    ...
    'RPG_prod_t+1':      'RPG',
    'Ethane_uncracked_t+1':  'Ethane',
    'Propane_uncracked_t+1': 'Propane',
    'Fuel_Gas': 'Fuel_Gas'
}

@dataclass(frozen=True)
class FuelGasConstants:
    cp_wavg_kcal_per_ton_K: float = 411_488.209
...
# RCOT schema, feed_cols, rcot_all_cols (paste your schema code here)

# ──── 0. RCOT / Feed schema ───────────────────────────────────────────────────
rcot_schema = {
    1: {'virt': ['RCOT_chamber1'],
        'real': {'RCOT_chamber1': ['RCOT_chamber1']}},
    2: {'virt': ['RCOT_chamber2'],
        'real': {'RCOT_chamber2': ['RCOT_chamber2']}},
    3: {'virt': ['RCOT_chamber3'],
        'real': {'RCOT_chamber3': ['RCOT_chamber3']}},
}
# Furnaces 4–6 each get two virtual knobs and map to 8 real channels
for ch in (4,5,6):
...
# flatten all real RCOT cols for bookkeeping
rcot_all_cols = []
for info in rcot_schema.values():
    for real_list in info['real'].values():
        rcot_all_cols.extend(real_list)
