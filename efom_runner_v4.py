...
if __name__ == "__main__":
    ...
    # --- drop current partial (09:00,21:00] bin so latest is last completed 09:00/21:00 ---
    _now = pd.Timestamp.now(tz='Asia/Seoul')
    _last_done = (_now - pd.Timedelta(hours=9)).floor('12H') + pd.Timedelta(hours=9)
    _last_done = _last_done.tz_localize(None)  # match tz-naive indices
    X_12h = X_12h.loc[:_last_done]
    Y_12h = Y_12h.loc[:_last_done]

    # 2) Build merged_lims from PI minute CSV (robust gas coalescing)
    EPS = 1e-6
    canon = ['Ethylene','Ethane','Propylene','Propane','n-Butane','i-Butane']
    ...
    # --- Merge gas_map (from X_12h) and treat all-zero gas vectors as missing so ffill can work
    ...
    if present:
        zero_vec = (lims[present].abs().sum(axis=1) <= EPS)
        lims.loc[zero_vec, present] = np.nan
        lims[present] = lims[present].ffill()

    # --- Backfill from Excel (merged_lims2) and coalesce GAS: prefer non-zero Excel over zero/NaN canonical
    ...
    # As-of backward (≤ 1 day) mapping from merged_lims2
    lims = _ensure_date_col(lims).sort_values('date').reset_index(drop=True)
    merged_lims2 = _ensure_date_col(merged_lims2).sort_values('date').reset_index(drop=True)
    ...
    if src_cols:
        ...
        # Coalesce PONA & per-carbon as before (limited fills)
        for c in [*pona_fam, *per_carbon_cols]:
            if c not in lims.columns:
                lims[c] = pd.NA
            lims[c] = pd.to_numeric(lims[c], errors='coerce')
            if c in ml2_map.columns:
                lims[c] = lims[c].where(lims[c].notna(), pd.to_numeric(ml2_map[c], errors='coerce'))
        fill_cols = [c for c in (pona_fam + per_carbon_cols) if c in lims.columns]
        if fill_cols:
            lims[fill_cols] = lims[fill_cols].ffill(limit=SHORT_FILL_LIMIT).bfill(limit=SHORT_FILL_LIMIT)

        # *** GAS coalesce: if canonical is NaN or near-zero, take Excel value when non-zero ***
        for c in canon:
            if c not in lims.columns:
                lims[c] = pd.NA
            lims[c] = pd.to_numeric(lims[c], errors='coerce')
            if c in ml2_map.columns:
                excel_v = pd.to_numeric(ml2_map[c], errors='coerce')
                need = lims[c].isna() | (lims[c].abs() <= EPS)
                lims.loc[need, c] = excel_v.loc[need].where(excel_v.abs() > EPS)

        # smooth short gaps on GAS cols
        g_present = [c for c in canon if c in lims.columns]
        if g_present:
            lims[g_present] = lims[g_present].ffill(limit=SHORT_FILL_LIMIT).bfill(limit=SHORT_FILL_LIMIT)

    # If *_x / *_y / *_gas leaked in, coalesce into canonical and drop them
    for c in canon:
    ...
    # Keep canonical gas for SRTO (drop any leftover *_gas)
    lims = _unify_gas_columns_for_srto(lims, keep_canon_only=True)

    # Final SRTO-sanitized merged_lims
